...
archivo_ssr = r"C:\\Users\\sfarf\\OneDrive\\PYTHON\\listado_ssr_nombre_real.xlsx"
ruta_base = r"G:\\Unidades compartidas\\CFC 2025\\2501_03_Diagnóstico SSR Región Biobío\\8. Etapa II\\Etapa II_SF"
carpeta_modelo = r"G:\\Unidades compartidas\\CFC 2025\\2501_03_Diagnóstico SSR Región Biobío\\8. Etapa II\\SSR166 - COPIULEMU"
...
with st.container():
    ...
    if st.session_state.modo == "carga":
        ...
        ruta_ssr = os.path.join(ruta_base, tipo_ssr)
        ...
        if st.button("Subir archivo(s)"):
            ...
            if st.button("Abrir carpeta de destino"):
    ...
    if st.session_state.modo == "lector":
        ...
        ruta_ssr_lector = os.path.join(ruta_base, tipo_ssr_lector)

        detected_subcarpetas_lector = [f.name for f in os.scandir(ruta_ssr_lector) if f.is_dir()]
        if not detected_subcarpetas_lector:
            st.warning("⚠️ No se encontraron subcarpetas en el SSR seleccionado.")
        ...
        if os.path.exists(ruta_actual_lector):
            ...
            if st.button("📂 Abrir carpeta de destino"):
                os.startfile(ruta_actual_lector)
